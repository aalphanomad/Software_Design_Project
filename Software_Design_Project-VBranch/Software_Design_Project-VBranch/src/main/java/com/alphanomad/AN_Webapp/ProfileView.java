				...
				event1 -> {
					...
					Button confirmPass = new Button("Confirm Password", 
							event2 -> {
								...
								else if(!the_password.equals(LoginView.get_SHA_1_SecurePassword(current.getValue(), student_number))) {
									valid=false;
							    ...
							    if(valid==true) {
									...
									String[] values = {LoginView.get_SHA_1_SecurePassword(confirm_new.getValue().toString() , student_number), student_number};
