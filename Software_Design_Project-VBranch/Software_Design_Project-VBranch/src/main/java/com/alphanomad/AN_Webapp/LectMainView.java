...
public class LectMainView extends VerticalLayout implements View
{
	...
	public void enter(ViewChangeEvent vc_event)
	{
		...
		//if the arraylist does have courses confirmed for the lecturer, then fill the combobox with these courses
 		...
 		//else, give a warning to the user that they do not have access yet
 			...
 			Label warn = new Label("<p style = \"font-family:georgia,garamond,serif;font-size:30px;\">\r\n"
 					+ "       <b><u>YOU DO NOT HAVE ACCESS. PLEASE WAIT FOR THE ADMIN TO APPROVE YOUR APPLICATION.</u></b> " + "      </p>", ContentMode.HTML);
 			addComponent(warn);
		...
		for (int i = 0; i < Course1.size(); i++)
		{
			...
			emails=emails+StudCourse1.get(i)+"@students.wits.ac.za;";			
		}
		if(emails.length()!=0)
		...
		combobox.addValueChangeListener(event ->
		{
			...
			case 1:
				for (int i = 0; i < Course2.size(); i++)
				{
					...
					emails=emails+StudCourse2.get(i)+"@students.wits.ac.za;";
				}
				if(emails.length()!=0)
				emails.substring(0, emails.length()-1);
				break;
			case 2:
				for (int i = 0; i < Course3.size(); i++)
				{
					...
					emails=emails+StudCourse3.get(i)+"@students.wits.ac.za;";
				}
				if(emails.length()!=0)
				emails.substring(0, emails.length()-1);
				break;
			case 3:
				for (int i = 0; i < Course4.size(); i++)
				{
					...
					emails=emails+StudCourse4.get(i)+"@students.wits.ac.za;";
				}
				if(emails.length()!=0)
				emails.substring(0, emails.length()-1);
			...
			case 4:
				for (int i = 0; i < Course5.size(); i++)
				{
					...
					emails=emails+StudCourse5.get(i)+"@students.wits.ac.za;";
				}
				if(emails.length()!=0)
				emails.substring(0, emails.length()-1);
				break;
		...
		*/
		...
		Button email_button = new Button("Email All Tutors",
				...
				event -> UI.getCurrent().getPage().open("mailto:" + emails, "_blank")
