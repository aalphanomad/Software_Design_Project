	...
	public void enter(ViewChangeEvent vc_event) {
		...
		Button updatePassword = new Button("Change User's Password", event1 -> {
			...
			Button confirmPass = new Button("Confirm Password", event2 -> {
				...
				try {
					String[] value = { "USER_INFORMATION", "STUDENT_NUM", "STUDENT_NUM",current.getValue(), student_num };
				...
				if (AdminPassword.isEmpty() == false && !the_password.equals(LoginView.get_SHA_1_SecurePassword(AdminPassword.getValue().toString(), student_num))) {
					valid = false;
				...
				if (valid == true) {
					...
					String[] val2 = { "USER_INFORMATION", "STUDENT_NUM", "STUDENT_NUM", current.getValue().toString(), student_num };
					...
					String[] values = { LoginView.get_SHA_1_SecurePassword(confirm_new.getValue().toString(), theStudentNum), theStudentNum };
