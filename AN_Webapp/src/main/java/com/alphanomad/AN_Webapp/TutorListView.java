...
public class TutorListView extends VerticalLayout implements View
{
	...
	public ComboBox<String> forMail=new ComboBox<>();
	...
	private void view_users()
	{
		...
		ArrayList<String>option_for_mailing=new ArrayList<>();
		option_for_mailing.add("All the Lectures");
		option_for_mailing.add("All the Tutors");
		option_for_mailing.add("Everyone");
		option_for_mailing.add("An individual");
		forMail.setItems(option_for_mailing);
		forMail.setPlaceholder("Send an email to:");
		forMail.setWidth("25%");
		horiz.addComponent(forMail);
		
		
		Button email_button = new Button("Send Email",event->{
			if(forMail.getValue().toString().equals("All the Lectures")) {
				DBHelper dbHelper=new DBHelper();
				String[] parameters = { "table", "target", "filter", "value" };
				String[] values2 = { "USER_INFORMATION", "EMAIL_ADDRESS", "ROLE", "1"};
				String result=dbHelper.php_request("generic_select", parameters, values2);
				
				ArrayList<String> emails = new ArrayList<>();
				String[] AllInfo = result.split("\"\\],\\[\"");
				AllInfo[0]=AllInfo[0].substring(3, AllInfo[0].length());
				
				
				String v=AllInfo[AllInfo.length-1];
				String[] h=v.split("\"\\]\\]");
				
				Notification.show(h[0]);
				
			    AllInfo[AllInfo.length-1]=h[0];
			    String theEmails="";
				for(int i=0;i<AllInfo.length;++i) {
					theEmails=theEmails+AllInfo[i]+";";
				}
				
			 UI.getCurrent().getPage().open("mailto:" + theEmails, "_blank");
			 
			}else if(forMail.getValue().toString().equals("All the Tutors")) {
				DBHelper dbHelper=new DBHelper();
				String[] parameters = { "table", "target", "filter", "value" };
				String[] values2 = { "USER_INFORMATION", "EMAIL_ADDRESS", "ROLE", "0"};
				String result=dbHelper.php_request("generic_select", parameters, values2);
			...
			}else if(forMail.getValue().toString().equals("Everyone")) {
				DBHelper dbHelper=new DBHelper();
				String[] parameters = { "table", "target", "filter", "value" };
				String[] values2 = { "USER_INFORMATION", "EMAIL_ADDRESS", "ROLE", "*"};
				String result=dbHelper.php_request("generic_select", parameters, values2);
			...
			}else if(forMail.getValue().toString().equals("An individual")) {
				Panel p = new Panel();
				p.setHeight("200px");
				p.setWidthUndefined();
				
				horiz.addComponent(p);
				...
				current.setCaption("Enter Users' Email:");
				fl.addComponent(current);
				
				Button Send = new Button("Send", 
						event2 -> {
							String email=current.getValue().toString();
							UI.getCurrent().getPage().open("mailto:" + email, "_blank");			
				});
				
				fl.addComponent(Send);
				p.setContent(fl);
			}	
		});
		horiz.addComponent(email_button);
		
		//UI.getCurrent().getPage().open("mailto:" + mail, "_blank");
